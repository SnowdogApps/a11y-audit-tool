...
const { data: axeResults } = await supabase
  .from('axe')
  .select('id, selector, results, size, form_data')
  .eq('audit_id', auditId)
...
const changeDevice = (value: string) => {
  ...
  device.value = value
  resultId.value = urlAndSelectorOptionsForSelectedDevice.value.find(
    ...
    <Accordion>
      <AccordionTab header="Audit Information">
        <ul class="space-y-1">
          ...
          <li><span class="font-bold">Status: </span>{{ auditInfo.status }}</li>
